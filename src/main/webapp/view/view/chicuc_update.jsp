...
<%--    <jsp:include page="/view/layout/link.jsp" />--%>
    ...
    <title>Document</title>
    <link rel="stylesheet" href="${pageContext.request.contextPath}/view/static/chicuc_update.css">
    <jsp:include page="/view/layout/link.jsp"/>
...
<div id="content" style="transition: margin-left .5s;">
    <div class="container mb-5 parent">
        <div class="mb-4">
            <h2><i class="far fa-edit"></i> Cập nhật danh sách quản lý cầu</h2>
        </div>
        <form action="" method="POST">
            <input name="oldSoThuTu" type="hidden" value="${cau.soThuTu}">
            <div class="form-group">
                <label for="userId">&nbsp; Chi cục / Sở GTVT</label>
                <input type="hidden" name="userId" value="${cau.user.userId}">
                <select disabled id="userId" class="form-control" required>
                    <option value="" disabled selected>Chọn chi cục / sở GTVT</option>
                    <c:forEach var="item" items="${userList}">
                        <option <c:if test="${item.userId ==cau.user.userId}">selected</c:if> value="${item.userId}">${item.soGTVT.ten}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="form-group">
